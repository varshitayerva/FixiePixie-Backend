...
import org.springframework.stereotype.Component;
    ...
    public class LoggingAspect {
        ...
        @Before("execution(* com.gl.project.UserService.service.*.*(..))")
        public void logBefore(JoinPoint joinPoint) {
            logger.info("Executing Service Method: {}", joinPoint.getSignature().getName());
        }

        @AfterReturning(pointcut = "execution(* com.gl.project.UserService.service.*.*(..))", returning = "result")
        public void logAfterReturning(JoinPoint joinPoint, Object result) {
            logger.info("Completed Service Method: {} with result: {}", joinPoint.getSignature().getName(), result);
        }

        @AfterThrowing(pointcut = "execution(* com.gl.project.UserService.service.*.*(..))", throwing = "error")
